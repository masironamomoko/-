...
public class NewsServicelmpt implements NewsService{
    ...
    public news_list_out NewsFindAll(news_findall_in newsFindallIn) throws SQLException {
        ...
        String sql="select news_id,date,title,user_name from news n1 left join user u1 on n1.author=u1.user_id where new_check=1 order by news_id desc limit "+(newsFindallIn.getPageNum()-1)*newsFindallIn.getPageSize()+","+newsFindallIn.getPageSize()+"";
    ...
    public List<News> NewsInquire() throws SQLException {
        String sql="select news_id,date,title,user_name from news n1 left join user u1 on n1.author=u1.user_id where new_check=1 order by news_id desc limit 0,4";
    ...
    public news_list_out NewsKeySearch(news_key_in news) throws SQLException {
        String sql="select news_id,date,title,user_name from news n1 left join user u1 on n1.author=u1.user_id where new_check=1 and title like '%"+news.getKey()+"%' order by news_id desc limit "+(news.getPageNum()-1)*news.getPageSize()+","+news.getPageSize()+"";
    ...
    public news_list_out NewsDateSearch(news_date_in news) throws SQLException{
        String sql="select news_id,date,title,user_name from news n1 left join user u1 on n1.author=u1.user_id where new_check=1 and  ( datediff ( date , '"+news.getDate()+"' ) = 0 ) order by news_id desc limit "+(news.getPageNum()-1)*news.getPageSize()+","+news.getPageSize()+"";
